...
class OOSF_ShortcodeManager {
	...
	function do_shortcode_tag( $m ) {
		...
		if ( is_array( $attr ) ) {
			if ( is_array( $attr['options'] ) ) {
				if ( $this->shortcode_tags[$tag]['has_name'] && ! empty( $attr['options'] ) ) {
					...
					if ( ! oosf_is_name( $scanned_tag['name'] ) )
			...
			if ( OOSF_USE_PIPE ) {
				$pipes = new OOSF_Pipes( $scanned_tag['raw_values'] );
		...
		$scanned_tag = apply_filters( 'oosf_form_tag', $scanned_tag, $this->exec );
	...
	function shortcode_parse_atts( $text ) {
		...
		if ( preg_match( $pattern, $text, $match ) ) {
			...
			if ( ! empty( $match[2] ) ) {
				...
				$atts['values'] = oosf_strip_quote_deep( $matched_values[0] );
			}
		} else {
			$atts = $text;
		}
...
$oosf_shortcode_manager = new OOSF_ShortcodeManager();

function oosf_add_shortcode( $tag, $func, $has_name = false ) {
	global $oosf_shortcode_manager;

	return $oosf_shortcode_manager->add_shortcode( $tag, $func, $has_name );
}

function oosf_remove_shortcode( $tag ) {
	global $oosf_shortcode_manager;

	return $oosf_shortcode_manager->remove_shortcode( $tag );
}

function oosf_do_shortcode( $content ) {
	global $oosf_shortcode_manager;

	return $oosf_shortcode_manager->do_shortcode( $content );
}

function oosf_get_shortcode_regex() {
	global $oosf_shortcode_manager;

	return $oosf_shortcode_manager->get_shortcode_regex();
}
