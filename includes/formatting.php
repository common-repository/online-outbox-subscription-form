...
function oosf_autop( $pee, $br = 1 ) {
	if ( trim($pee) === '' )
		return '';
	$pee = $pee . "\n"; // just to make things a little easier, pad the end
	$pee = preg_replace( '|<br />\s*<br />|', "\n\n", $pee );
	// Space things out a little
	/* oosf: removed select and input */
...
function oosf_strip_quote( $text ) {
	$text = trim( $text );
...
function oosf_strip_quote_deep( $arr ) {
	if ( is_string( $arr ) )
		return oosf_strip_quote( $arr );

	if ( is_array( $arr ) ) {
		$result = array();
		foreach ( $arr as $key => $text ) {
			$result[$key] = oosf_strip_quote( $text );
		}
		return $result;
	}
}

function oosf_canonicalize( $text ) {
...
function oosf_sanitize_file_name( $filename ) {
	/* Memo:
	...
	// it in turn calls sanitize_file_name(). Therefore this oosf_sanitize_file_name() will be
...
function oosf_is_name( $string ) {
	...
	return preg_match( '/^[A-Za-z0-9][-A-Za-z0-9_:.]*$/', $string );
}
