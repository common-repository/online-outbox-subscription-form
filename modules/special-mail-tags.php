...
add_filter( 'oosf_special_mail_tags', 'oosf_special_mail_tag', 10, 2 );

function oosf_special_mail_tag( $output, $name ) {

	// For backwards compat.
	$name = preg_replace( '/^oosf\./', '_', $name );

	if ( '_remote_ip' == $name )
		$output = preg_replace( '/[^0-9a-f.:, ]/', '', $_SERVER['REMOTE_ADDR'] );

	elseif ( '_url' == $name )
		$output = get_option( 'home' ) . oosf_get_request_uri();
...
add_filter( 'oosf_special_mail_tags', 'oosf_special_mail_tag_for_post_data', 10, 2 );

function oosf_special_mail_tag_for_post_data( $output, $name ) {

	if ( ! isset( $_POST['_oosf_unit_tag'] ) || empty( $_POST['_oosf_unit_tag'] ) )
		return $output;

	if ( ! preg_match( '/^oosf-f(\d+)-p(\d+)-o(\d+)$/', $_POST['_oosf_unit_tag'], $matches ) )
		return $output;
